	...
	class iworks_rate {
		...
		public function add_donate_link( $actions, $plugin_file, $plugin_data, $context ) {
			...
			if ( ! empty( $settings_page_url ) ) {
				$actions['settings'] = sprintf(
					...
					esc_html__( 'Settings', 'simple-revision-control' )
				);
			}
			$actions['donate'] = sprintf(
				...
				esc_html__( 'Provide us a coffee', 'simple-revision-control' )
			);
			return $actions;
		}
		...
		private function get_plugin_data_by_plugin_id( $plugin_id ) {
			if ( ! isset( $this->plugins[ $plugin_id ] ) ) {
				return new WP_Error(
					...
					sprintf(
						...
						esc_html__( 'There is no plugin with id: %s.', 'simple-revision-control' ),
						$plugin_id
					)
				);
			}
			...
			$plugin['url'] = esc_url(
				...
				/* translators: %s: plugin slug */
					_x( 'https://wordpress.org/plugins/%s', 'plugins home', 'simple-revision-control' ),
					$plugin['slug']
				)
			);
			$plugin['support_url'] = esc_url(
				sprintf(
					/* translators: %s: plugin slug */
					_x( 'https://wordpress.org/support/plugin/%s', 'plugins support home', 'simple-revision-control' ),
					$plugin['slug']
				)
			);
			/**
		...
		public function filter_get_advertising_og( $data ) {
			return array(
				'iworks-adverting-og' => array(
					'title'    => __( 'OpenGraph', 'simple-revision-control' ),
		...
		public function get_advertising_og_content() {
			$args = array(
				...
				'plugin_name'        => __( 'OG — Better Share on Social Media', 'simple-revision-control' ),
				'plugin_wp_home'     => __( 'https://wordpress.org/plugins/og/', 'simple-revision-control' ),
			);
