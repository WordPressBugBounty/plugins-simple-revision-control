...
class iworks_options {
		...
		foreach ( $pages as $key => $data ) {
			...
			$data = wp_parse_args(
				...
				array(
					...
					'icon_url'   => null,
					'parent'     => null,
					'page_title' => esc_html__( 'No Page Title', 'simple-revision-control' ),
				)
			);
			/**
			 * Check callback
	...
	public function build_options( $option_group = 'index', $echo = true, $term_id = false ) {
		...
		if ( ! is_array( $options['options'] ) ) {
			...
			esc_html_e( 'An error occurred while getting the configuration.', 'simple-revision-control' );
			echo '</strong></p></div>';
			return;
		}
		/**
			...
			switch ( $option['type'] ) {
				...
				case 'image':
					...
					$content .= sprintf(
						...
						esc_attr__( 'Select Image', 'simple-revision-control' ),
						esc_attr( $html_element_name )
					);
					$content .= sprintf(
						' <input type="button" class="button iworks_delete_button" value="%s" rel="#%s" %s/>',
						esc_attr__( 'Delete image', 'simple-revision-control' ),
						esc_attr( $html_element_name ),
		...
		if ( $options['show_submit_button'] ) {
			$content .= get_submit_button(
				esc_html__( 'Save Changes', 'simple-revision-control' ),
				'primary',
				'submit_button'
			);
		}
...
<?php } ?>
	...
	private function location( $name, $value = '', $args = array() ) {
		...
		$i18n     = array(
			'country' => esc_html__( 'Country', 'simple-revision-control' ),
			'city'    => esc_html__( 'City', 'simple-revision-control' ),
			'street'  => esc_html__( 'Street', 'simple-revision-control' ),
			'zip'     => esc_html__( 'ZIP code', 'simple-revision-control' ),
		);
		...
		$f = array(
			...
			array(
				...
				'wp_localize_script' => array(
					'buttons' => array(
						'select_media' => __( 'Select Image', 'simple-revision-control' ),
					),
				),
			),
			/**
	...
	public function get_switch_button_data() {
		$data = array(
			'labels' => array(
				'off_label' => esc_html__( 'OFF', 'simple-revision-control' ),
				'on_label'  => esc_html__( 'ON', 'simple-revision-control' ),
			),
		);
		return $data;
	}
	...
	private function get_nonce_value() {
		...
		return new WP_Error( 'security', esc_html__( 'Failed Security Check', 'simple-revision-control' ) );
	}
